...
func benchmarkRangeScans(b *testing.B, db DB, dbSize int64) {
	...
	for i := 0; i < b.N; i++ {
		start := rand.Int63n(dbSize - rangeSize) //nolint:gosec // G404: Use of weak random number generator
		end := start + rangeSize
...
func benchmarkRandomReadsWrites(b *testing.B, db DB) {
	...
	for i := 0; i < b.N; i++ {
		// Write something
		{
			idx := rand.Int63n(numItems) //nolint:gosec // G404: Use of weak random number generator
			internal[idx]++
			val := internal[idx]
